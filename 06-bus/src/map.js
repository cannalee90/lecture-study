...
const { fromEvent, of, combineLatest, from } = rxjs;
const { ajax } = rxjs.ajax;
const { map, tap, switchMap, pluck, scan, mergeMap } = rxjs.operators;
...
export default class Map {
    ...
    createDragend$() {
        return fromEvent(this.naverMap, 'dragend')
            .pipe(
                ...
                tap(coor => console.log('createDragend', coor))
            );
    }
    ...
    mapStation(coord$) {
        return coord$.pipe(
            ...
            tap(res => console.log('mapStateion', res)),
        )
    }
    ...
    constructor($map) {
        ...
        //     tap(a => console.log('busRouteList', a)),
        // ).subscribe();

        // this.createDragend$()
        //     .pipe(this.mapStation)
        //     .subscribe(stations => {
        //         console.log('subs', stations);
        //         if (this.markders.length)  {
        //             this.markders.forEach(marker => this.deleteMarker(marker))
        //         }
        //         this.markders = stations.map(station => this.createMarker(station.stationName, station.x, station.y))
        //     })
    }
}